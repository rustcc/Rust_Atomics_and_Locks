...
另一种方式是通过*泄漏*分配内存的方式共享所有权。使用 `Box::leak`，人们可以释放 `Box` 的所有权，保证永远不会 drop 它。从那时起，`Box` 将永远存在，没有所有者，只要程序运行，任意线程都可以借用它。
...
3. 两个线程通过各自的 Arc 共访问享数组。当它们 drop 它们的 Arc 时，两者都会减少引用计数。最后一个 drop 它的 Arc 的线程将看见计数器减少到 0，并且将是 drop 和回收数组的内存。
...
* 我们必须改变一些事情：
  ...
  * 解锁、等待以及重新锁定都是通过 `wait` 方法完成的。我们不得不稍微重组控制流，以便传递 guard 到 wait 方法，同时在处理项之前仍然 drop 它。
