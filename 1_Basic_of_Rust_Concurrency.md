...
消费线程进行一个无穷尽的循环，它将项弹出队列，使用 `dbg` 宏展示它们。当队列为空的时候，它停止并且使用 `park()` 函数进行睡眠。如果它得到解锁，`park()` 调用将返回，循环继续，再次从队列中弹出项，直到它是空的。等等。

生产线程将其推入队列，每秒产生一个新的数字。每次增加一个项时，它都会在 Thread 对象上使用 `unpark()` 方法，该方法引用消费线程来解锁它。这样，消费线程就会被唤醒处理新的元素。

这样，我们要作出的一个重要的观测是，如果我们移除**阻塞**，程序将在理论上正确，尽管效率低下。这是重要的，因为 `park()` 不能保证它将由于匹配 `unpark()` 而返回。尽管有些罕见，但它很可能会有*虚假唤醒*。我们的示例处理得很好，因为消费线程将锁定去咧，可以看到它是空的，然后直接解锁它并再次阻塞。
