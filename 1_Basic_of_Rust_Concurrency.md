...
Rust 标准库通过 `std::rc::Rc` 类型提供了该功能，它是“引用计数”（reference counted）的缩写。它与 `Box` 非常类似，唯一的区别是克隆它将不会分配任何新内存，而是递增存储在包含值旁边的计数器。原始的 `Rc` 和克隆的 `Rc` 将引用相同内存分配；它们*共享所有权*。
...
* 常规的线程可以像程序运行一样长时间，并且因此只能借用 `'static` 数据。例如静态变量和泄漏内存分配。
