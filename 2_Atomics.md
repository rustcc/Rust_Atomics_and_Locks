...
> 如果原子变量从某个值 A 更改到 B，然后又变回 A，但在 load 操作之后和 compare_exchange 操作之前，它仍然会成功，即使原子变量在此期间被更改（并且更改回来）。在很多示例中，就像在我们的加运算示例中一样，这并不是一个问题。然而，有几种算法，通常涉及原子指针，这被称为 <a id="index-ABAproblem"></a> ABA 问题。
